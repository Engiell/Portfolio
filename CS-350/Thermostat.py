...
#    2          Added functions to buttons, lights, and LCD display
#------------------------------------------------------------------
...
class ManagedDisplay():
    ...
    def updateScreen(self, line1, line2):
        """
        Write two lines to the display (with built-in truncation).
        """
        self.lcd.clear()
        message = f"{line1[:16]}\n{line2[:16]}"
        if DEBUG:
            print("LCD message =>", repr(message))
...
class TemperatureMachine(StateMachine):
    ...
    def __init__(self):
        super().__init__()
        self.setPoint     = TemperatureMachine.setPoint
        self._last_temp_F = 72.0
    ...
    def getFahrenheit(self):
        #Reads temperature and converts to Fahrenheit.
        #Uses last known temp if I²C read fails.
        try:
            ...
            if DEBUG:
                print(f"I2C error, using last temp: {e}")
            return self._last_temp_F
    
    ##
    ##  Configure output string for the Thermostat Server
    ##
    def setupSerialOutput(self):
        t = floor(self.getFahrenheit())
    ...
    def manageMyDisplay(self):
        ...
        while not self.endDisplay:
            ...
            ## Line 1
            now = datetime.now()
            lcd_line_1 = now.strftime("%Y-%m-%d %H:%M").ljust(16)
            ## Line 2
            temp = floor(self.getFahrenheit())
            if(altCounter < 6):
                ...
                lcd_line_2 = f"{st} @ {self.setPoint:2d}F".ljust(16)
                altCounter = altCounter + 1
                if(altCounter >= 11):
